...
export default function AddPackagePage() {
    ...
    const handleSave = async () => {
        if (!form.package_name || !form.validity_duration || !form.cost || !form.description || !form.image || form.services.length === 0) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-20 px-4 sm:px-0">
            ...
            <div className="flex flex-col gap-4">
                <Link href="/packages" className="flex items-center gap-2 text-slate-500 hover:text-blue-600 font-medium transition-colors w-fit">
                    <ArrowLeft className="w-4 h-4" /> Back to Packages
                </Link>
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Create Healthcare Package</h1>
                        <p className="text-slate-500 mt-1 font-medium">Bundle multiple services into a single subscription plan.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm space-y-4">
                        <label className="text-sm font-bold text-slate-700 block ml-1">Package Image <span className="text-red-500">*</span></label>
                        <div
                            ...
                            {imagePreview ? (
                                ...
                                <>
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-[10px] font-medium text-slate-400 mt-0.5">PNG, JPG up to 5MB</p>
                                    </div>
                                </>
                            )}
                        </div>
                    ...
                    <div className="bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm space-y-6">
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-slate-700 ml-1 flex items-center gap-2">
                                <Clock className="w-4 h-4 text-blue-500" /> Validity (Days) <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="number"
                                placeholder="e.g. 30"
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-slate-700 ml-1 flex items-center gap-2">
                                <DollarSign className="w-4 h-4 text-emerald-500" /> Package Cost ($) <span className="text-red-500">*</span>
                            </label>
                            <input
                                type="number"
                ...
                {/* Right Column: Details & Service Selection */}
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm space-y-8">
                        {/* Basic Details */}
                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700 ml-1">Package Name <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-slate-700 ml-1">Package Description <span className="text-red-500">*</span></label>
                                <textarea
                        ...
                        <div className="pt-6">
                            <button
                                disabled={isLoading}
                                className="w-full bg-blue-600 text-white rounded-2xl py-5 font-black hover:bg-blue-700 transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-100 active:scale-[0.98] disabled:opacity-70 text-lg"
                                onClick={handleSave}
                            >
                                {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : <Save className="w-6 h-6" />}
                                <span>{isLoading ? "Creating Package..." : "Launch This Package"}</span>
                            </button>
                        </div>
                    </div>
                </div>
